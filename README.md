...
3. Run the arena with `python src/run_court_arena.py --num-cases <integer>`. This will randomly sample the specified number of cases and perform the CourtArena simulation process. Model outputs for each case will be stored in `/logs`, and model evaluation results and other useful info will be stored in `/results`. Additionally, calling this script with `--num-cases all` will run the arena on all available cases in `case_data/courtreasoner_cases.json`.
