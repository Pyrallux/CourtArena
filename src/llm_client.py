...
Dual-path design:
- OllamaClient: Native ollama.chat() preserving all 6 nanosecond timing fields. Supports local and cloud Ollama instances (via OLLAMA_HOST).
- LiteLLMClient: litellm.acompletion() for commercial APIs (OpenAI, Google, xAI) and other cloud models.

Both return a unified LLMResponse that produces metadata dicts.
"""
...
class OllamaClient:
    ...
    async def chat(self, messages: List[Dict[str, str]],
        ...
        if "cloud" in effective_model:
            ...
            # Force local host for non-cloud models to override a global OLLAMA_HOST in .env
            client_kwargs["host"] = "http://127.0.0.1:11434"

        sync_client = OllamaClientLib(**client_kwargs)
...
def _is_ollama_model(model_name: str) -> bool:
    """Detect whether a model name is an Ollama model.

    Ollama models look like 'gemma2:9b-instruct-q4_K_M' or end with ':cloud' — they contain ':'
    but usually do not contain '/'. Cloud-routed models (OpenRouter, etc.)
    often contain '/' in their provider prefix, unless accessed over LiteLLM's `ollama/...`.
    """
    if "/" in model_name:
        return False
    if ":" in model_name:
...
def create_client(model_name: str, temperature: float = 0.7,
                  max_tokens: int = 1024, timeout: float = 900.0,
                  reasoning_effort: Optional[str] = None):
    """Factory: returns OllamaClient for local or cloud Ollama models, else LiteLLMClient."""
    # Special handling if the model name implies LiteLLM's format for Ollama
    if model_name.startswith("ollama/"):
        return LiteLLMClient(model_name, temperature, max_tokens, timeout,
                             reasoning_effort=reasoning_effort)
    
    if _is_ollama_model(model_name) or ":cloud" in model_name:
